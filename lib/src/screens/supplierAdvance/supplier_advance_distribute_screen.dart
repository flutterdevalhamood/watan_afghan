// distribute_supplier_advance_screen.dart
...
class SupplierAdvanceDistributeScreen extends StatefulWidget {
  final SupplierAdvance supplierAdvance;

  const SupplierAdvanceDistributeScreen({
    super.key,
    required this.supplierAdvance,
  });

  @override
  State<SupplierAdvanceDistributeScreen> createState() =>
      _SupplierAdvanceDistributeScreenState();
}

class _SupplierAdvanceDistributeScreenState
    extends State<SupplierAdvanceDistributeScreen> {
  SupplierAdvanceController? _controller;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _controller?.getSupplierInvoicesForDistribution(
        widget.supplierAdvance.supplier?.id ?? 0,
        widget.supplierAdvance.currency?.id ?? 1,
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Distribute Supplier Advance'),
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                'Supplier: ${widget.supplierAdvance.supplier?.Name ?? 'Unknown'}',
          ...
          Row(
            ...
            children: [
              ...
              Text(
                '${widget.supplierAdvance.currency?.Name ?? 'USD'} ${_formatAmount(widget.supplierAdvance.remainingBalance)}',
                style: TextStyle(
                  fontSize: 16,
  ...
  Widget _buildContent(SupplierAdvanceController controller) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                onPressed:
                    () => controller.getSupplierInvoicesForDistribution(
                      widget.supplierAdvance.supplier?.id ?? 0,
                      widget.supplierAdvance.currency?.id ?? 1,
                    ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'There are no pending invoices for this supplier',
                textAlign: TextAlign.center,
    ...
    return _buildInvoicesList(controller.supplierInvoicesForDistribution!);
  }

  Widget _buildInvoicesList(List<SupplierInvoiceForDistribution> invoices) {
    return Column(
      children: [
        // Select All Header
        Container(
  ...
  Widget _buildInvoiceCard(SupplierInvoiceForDistribution invoice) {
  ...
  Widget _buildBottomActionBar(SupplierAdvanceController controller) {
    final selectedAmount = _calculateSelectedAmount();
    final availableAmount =
        double.tryParse(widget.supplierAdvance.remainingBalance) ?? 0.0;
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${widget.supplierAdvance.currency?.Name ?? 'USD'} ${selectedAmount.toStringAsFixed(2)}',
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
                          color:
  ...
  void _handleSelectAll(bool? value) {
    setState(() {
      ...
      if (_selectAll) {
        final invoices = _controller?.supplierInvoicesForDistribution ?? [];
  ...
  void _toggleInvoiceSelection(int invoiceId) {
    setState(() {
      ...
      final totalInvoices =
          _controller?.supplierInvoicesForDistribution?.length ?? 0;
  ...
  void _handleDistribute(SupplierAdvanceController controller) {
    if (_selectedInvoices.isEmpty) {
      showErrorSnack('Please select atleast one invoice');
      return;
    }
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Are you sure you want to distribute this advance to ${_selectedInvoices.length} selected invoice(s)?',
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      'Distribution Amount: ${widget.supplierAdvance.currency?.Name ?? 'USD'} ${_calculateSelectedAmount().toStringAsFixed(2)}',
                      style: TextStyle(
                        fontWeight: FontWeight.w600,
                        color: Theme.of(context).primaryColor,
                      ),
  ...
  void _processDistribution(SupplierAdvanceController controller) async {
    final success = await controller.postSupplierAdvanceDistributionSave(
      supplierAdvanceId: widget.supplierAdvance.id,
          ...
          SnackBar(
            content: const Row(
              children: [
                ...
                SizedBox(width: 8),
                Text('Advance disbursement processed successfully.'),
              ],
            ),
