...
import 'package:sample/src/screens/customerPayment/customer_payment_bottom_sheet.dart';
...
class _CustomerPaymentListScreenState extends State<CustomerPaymentListScreen>
  ...
  void _setupScrollListener() {
    _scrollController.addListener(() {
      if (_scrollController.position.pixels ==
        ...
        _controller = Provider.of<CustomerPaymentController>(
          context,
          listen: false,
        );
        if (_controller.hasMore && !_controller.isLoading) {
          _controller.getCustomerPayment(loadMore: true);
        }
      }
    });
  }
  ...
  void _loadInitialData() {
    ...
    if (!_controller.hasData) {
      _controller.getCustomerPayment();
    }
  }

  Future<void> _handleRefresh() async {
    _searchController.clear();
    final provider = Provider.of<CustomerPaymentController>(
      context,
      listen: false,
    );
  ...
  Future<void> _handlePushPayment(CustomerPayment payment) async {
    ...
    try {
      await provider.getCustomerPaymentPush(payment.id);

      if (provider.pushErrorMessage == null) {
        await provider.getCustomerPayment();

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Customer payment pushed successfully'),
  ...
  Future<bool?> _showPushConfirmation(CustomerPayment payment) async {
    _pushReasonController.clear();
    return await showDialog<bool>(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Push Customer Payment'),
          content: Column(
  ...
  Widget _buildSearchBar(CustomerPaymentController provider) {
    return Container(
  ...
  Widget _buildContent(CustomerPaymentController provider) {
    return Column(
      children: [
        // Sticky info banner
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'If a customer has an unpushed payment entry, they will not be visible for creating a new payment entry until the previous one is pushed.',
                  style: TextStyle(
                    fontSize: 13,
            ...
            child: CustomScrollView(
              ...
              slivers: [
                if (provider.isLoading &&
                    provider.filteredCustomerPayments.isEmpty)
                  SliverFillRemaining(
                    child: Center(
                      child: Column(
                ...
                else if (provider.filteredCustomerPayments.isEmpty)
                  SliverFillRemaining(
                    child: Center(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ...
                          Text(
                            _searchController.text.isNotEmpty
                                ...
                                : 'No customer payments available',
                            style: TextStyle(
                  ...
                  SliverPadding(
                    ...
                    sliver: SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          if (index ==
                              provider.filteredCustomerPayments.length) {
                            return _buildLoadMoreIndicator(provider);
                          }
                          final payment =
                              provider.filteredCustomerPayments[index];
  ...
  Widget _buildPaymentCard(
    ...
    CustomerPaymentController provider,
  ) {
  ...
  void _showDeleteConfirmation(CustomerPayment payment) async {
    await DeleteConfirmationDialog.show(
      ...
      onDelete: (reason) async {
        await _controller.deleteCustomerPayment(payment.id, reason);
      },
          ...
          (context) => DraggableScrollableSheet(
            ...
            builder:
                (context, scrollController) =>
                    CustomerPaymentBottomSheet(id: payment.id),
          ),
    );
  }

  Widget _buildLoadMoreIndicator(CustomerPaymentController provider) {
    return Container(
      padding: const EdgeInsets.all(16),
