...
class _SupplierStatementScreenState extends State<SupplierStatementScreen> {
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      final isSuccess = await _controller.postSupplierStatement(
        fromDateFormatted,
        toDateFormatted,
        _selectedCurrencyId,
        _selectedSupplierId,
      ...
      if (isSuccess && _controller.supplierStatementUrl != null) {
        final pdfPath = await PdfDownloadHelper.downloadAndOpenPdf(
          url: _controller.supplierStatementUrl!,
            ...
            IconButton(
              ...
              onPressed:
                  () => PdfShareHelper.sharePdf(
                    context: context,
                    pdfPath: _pdfPath!,
                    subject: 'Supplier Statement Report',
                    text:
                        'Supplier Statement Report from ${DateFormat('MMM dd, yyyy').format(_fromDate!)} to ${DateFormat('MMM dd, yyyy').format(_toDate!)}',
                  ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    isExpanded: true,
                    items: [
                      ..._purchaseController.supplier?.map((currency) {
                            return DropdownMenuItem<int>(
                              value: currency['id'],
                              child: Text(currency['Name']),
                            );
